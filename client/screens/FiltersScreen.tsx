...
export default function FiltersScreen({ navigation }: ScreenProps) {
	...
	const handlePreferencePress = (key: PreferencesType) => {
		if (preferences.includes(key)) {
			setPreferences(
				preferences.filter((preference) => preference !== key)
			);
		} else {
	...
	return (
		<View style={styles.container}>
			...
			>
				{preferencesOptions.map((preference) => (
					<Chip
						...
						disabled={preference.disableKeys.some((key) => preferences.includes(key))}
					>
						{preference.text}
					</Chip>
				))}
			...
			<Button mode="contained" onPress={handleApplyClick}>Apply</Button>
		</View>
	);
}
